...
async function fetchPanorama(id: string): Promise<PanoramaImage | null> {
  ...
  try {
    ...
    if (!response.ok) {
      console.error('Failed to load panorama metadata for head (library):', response.statusText);
      return null;
    }

    return (await response.json()) as PanoramaImage;
  } catch (error) {
    console.error('Unexpected error loading panorama metadata for head (library):', error);
    return null;
  }
}
